    ...
    def _load_model(self, model_path):
        ...
        try:
            ...
            if isinstance(checkpoint, dict):
                # Case 1: Saved as state_dict or checkpoint dictionary
                if 'model_state_dict' in checkpoint:
                    # Saved with additional info (optimizer, epoch, etc.)
            ...
            logger.error("Make sure your model file contains either:")
            logger.error("1. A complete model object, or")
            logger.error("2. A state_dict that matches your model architecture")
            sys.exit(1)
    
    def _create_model_architecture(self):
        """
        Recreate your model architecture here.
        This is needed when loading from state_dict.
        Replace this with your actual model class and parameters.
        """
        ...
        class YogaPoseNet(nn.Module):
            def __init__(self, input_size=132, hidden_size=128, num_classes=len(self.label_map) if hasattr(self, 'label_map') and self.label_map else 10):
        ...
        model = YogaPoseNet(input_size=132, hidden_size=128, num_classes=num_classes)
        
        logger.info(f"Created model architecture with {num_classes} classes")
        return model
